...
class DualModelCore:
    ...
    def recommend(self, state: SharedFeatureState) -> dict:
        ...
        return {
            ...
            "speed_kph": speed,
            "rpm": int(getattr(state, "rpm", 11200)),
            "gear": int(getattr(state, "gear", 7)),
            "throttle": int(getattr(state, "throttle", 85)),
            "brake": int(getattr(state, "brake", 0)),
            "tyre_temps": getattr(state, "tyre_temps", {"fl": 102.4, "fr": 101.8, "rl": 98.2, "rr": 97.9}),
            "telemetry_source": getattr(state, "telemetry_source", "REPLAY_MODE"),
