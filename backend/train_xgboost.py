...
def main():
    ...
    parser.add_argument("--out", type=str, default="models/xgb_overtake.json")
    args = parser.parse_args()

    csv_path = args.data_csv
    if not csv_path:
        for candidate in ["data/telemetry.csv", "../data/telemetry.csv", "backend/data/telemetry.csv"]:
    ...
    if csv_path:
        ...
        if "gap_distance_m" not in df.columns and "gap_sec" in df.columns:
            speed = df.get("speed_kph", 250.0)
            df["gap_distance_m"] = df["gap_sec"] * (speed / 3.6)
        if "soc" not in df.columns and "battery_soc_pct" in df.columns:
            df["soc"] = df["battery_soc_pct"] / 100.0
        if "tyre_age_delta" not in df.columns and "tyre_age" in df.columns:
            df["tyre_age_delta"] = df["tyre_age"] - df["tyre_age"].median()
        if "battery_cost_mj" not in df.columns:
            if "energy_used_lap_mj" in df.columns:
                df["battery_cost_mj"] = df["energy_used_lap_mj"].clip(0.1, 4.0)
            else:
                df["battery_cost_mj"] = 2.0
        if "drs_available" in df.columns:
            df["drs_available"] = df["drs_available"].astype(int)
        if "overtake_success" in df.columns:
            df["overtake_success"] = df["overtake_success"].astype(int)
    else:
